...
public class P819MostCommonWords {
    ...
    public static String mostCommonWord(String paragraph, String[] banned) {
        ...
        for (String s : words) {
            if (repeats.get(s) == null) {
                repeats.put(s, 1);
                continue;
            }
            repeats.put(s, repeats.get(s) + 1);
        }
        System.out.println(repeats);
        int maxWordCount = 0;
        String maxWord = "";
        for (Map.Entry<String, Integer> entry : repeats.entrySet()) {
            if (isBanned(entry.getKey(), bannedSet)) {
                continue;
            }
    ...
    public static boolean isBanned(String word, Set<String> bannedSet) {
